...
export class UI {
  constructor({uiLoading, uiScanning, uiError, container}) {
    const cssBlock=document.createElement('style');
    cssBlock.innerText=css;
    document.head.appendChild(cssBlock);
    if (uiLoading === 'yes') {
      this.loadingModal = this._loadHTML(loadingHTML, container);
    } else if (uiLoading !== 'no') {
      this.loadingModal = document.querySelector(uiLoading);
    }

    if (uiError === 'yes') {
      this.compatibilityModal = this._loadHTML(compatibilityHTML, container);
    } else if (uiError !== 'no') {
      this.compatibilityModal = document.querySelector(uiError);
    }

    if (uiScanning === 'yes') {
      this.scanningMask = this._loadHTML(scanningHTML, container);
